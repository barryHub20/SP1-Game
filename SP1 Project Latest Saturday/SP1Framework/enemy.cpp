...
void enemyMovement(vector<enemy>& unit, double elapsedTime, COORD consoleSize, int size, int points, int waveCounter, double velocity, double frameTime)
{
	...
	if(elapsedTime >=spawnTime)
	{
		...
		{
			if(unit[i].active == true && unit[i].mobile == true)//if the unit is active
			{
...
void enemyUnitAi(double elapsedTime, enemy& unit, double velocity, int numberOfPoints, double frameTime)
{
	...
	if(unit.location.X != unit.point[unit.count].X && unit.location.Y == unit.point[unit.count].Y && unit.time >= velocity)
	{
	...
	else if(unit.time > velocity)//if the unit.time is longer than velocity
	{
		unit.time -= velocity;
	}
}
...
void targetPoint(vector<enemy>& unit, int points, COORD consoleSize)
{
